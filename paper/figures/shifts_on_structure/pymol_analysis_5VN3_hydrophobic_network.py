"""
This is a PyMOL script for analyzing shifts in preferences on the structure 5VN3

This script is similar to `pymol_analysis_5FYL.py`, but performs the analysis on
the structure 5VN3 instead of 5FYL.

This script intended to be run in the PyMOL terminal as:
    run pymol_analysis_5VN3.py
...
# In the original script `pymol_analysis_5FYL.py`, the `structure` variable was
# the name of a `PyMOL` object of the gp160 monomer. For this structure, it is
# the name of an object of the entire trimer. So that the rest of the code is
# compatible with this new naming system, I will rename the trimer object and
# then change the monomer object to equal `structure`.
...
# Show significant sites as spheres, hiding heteroatoms
print ("\nThere are {0} sites with significant RMSD corrected values".format(len(sig_sites)))
print (', '.join(sig_sites))
cmd.select('sig_RMSDcorrected', 'resi {0}'.format('+'.join(sig_sites)))
...
cmd.set_view ("""\
     0.603347421,   -0.174357295,    0.778173447,\
     0.788299561,    0.277918309,   -0.548923314,\
    -0.120561823,    0.944625914,    0.305132717,\
...
cmd.select('subs', 'resi {0}'.format('+'.join(substituted_sites)))
...
# Find sites that are both subsituted and shifted
substituted_and_shifted_sites = [site for site in substituted_sites if site in sig_sites]
cmd.select('shifted_and_sub', 'resi {0}'.format('+'.join(substituted_and_shifted_sites)))
